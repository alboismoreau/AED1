...
def de_filas_a_columnas(m:[[int]]) -> [[int]]:
    ...
    while j < (len(m[0])):
        for i in range (len(m)):
            columna_actual.append(m[i][j])
        res.append(columna_actual)
        columna_actual = []
        j += 1
    return res

def columnas_repetidas(m:[[int]]) -> [[int]]:
    columnas:[[int]] = de_filas_a_columnas(m)
    primera_mitad: [[int]] = []
    segundo_mitad: [[int]] = []

    for i in range(len(columnas) // 2):
        primera_mitad.append(columnas[i])
    for j in range(len(columnas) // 2, len(columnas), 1):
        segundo_mitad.append(columnas[j])
    
    return primera_mitad == segundo_mitad

#print(de_filas_a_columnas([[1,2,1,2],[-5,6,-5,6],[0,1,0,1]]))
#print(columnas_repetidas([[1,2,1,2],[-5,6,-5,6],[0,1,0,1]]))
#print(columnas_repetidas([[1,2,1,2],[-5,6,-5,6],[0,1,0,3]]))

#EJERCICIO 4------------------------------------------------------------------

naciones:[str] = ["arg", "aus", "nz", "sud"]
torneos : dict = {2023:["nz", "sud", "arg", "aus"], 2022:["nz", "sud", "aus", "arg"]}
